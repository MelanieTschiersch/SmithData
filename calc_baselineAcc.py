...
def decode_continuous_prev_cur(dataframe):
    ...
    for delta_t_train in range(len(df_serial['bin_sp_prev'][df_serial['bin_sp_prev'].index[0]])):# for each period ['targ_on', 'targ_off', 'go_cue', 'saccade', 'reward']
        ...
        for k in range(0,1000):# k=10 as in Barbosa2020
            #train test split
            rand_state = randint(0,100000)# to get the same split for previous, current
            ...
            # R squared value
            r2_crosscorr_prev.append(metrics.r2_score(preds_prev, y_test))
            r2_crosscorr_curr.append(metrics.r2_score(preds_curr, y_test))
        ...
        r2_acc_prev.append(np.mean(r2_crosscorr_prev))
        r2_std_prev.append(np.std(r2_crosscorr_prev))
        r2_acc_curr.append(np.mean(r2_crosscorr_curr))
        r2_std_curr.append(np.std(r2_crosscorr_curr))
    ...
    return r2_acc_prev, r2_std_prev, r2_acc_curr, r2_std_curr,\
        mse_acc_prev, mse_std_prev, mse_acc_curr, mse_std_curr
...
#only use Sa, sess0
df_Sa0 = df_dat.loc[(df_dat['monkey']=='Sa') & (df_dat['session']==0)]
...
for trial,idx in enumerate(df_Sa0.index[:-1]):
    if ((df_Sa0['trial_id'][idx]+1) == (df_Sa0['trial_id'][idx+1])):
        ...
        serial['response_prev'].append(round(df_Sa0['saccade_angle'][idx], 5))
        ...
        serial['response_curr'].append(round(df_Sa0['saccade_angle'][idx+1], 5))
...
r2_acc_prev, r2_std_prev, r2_acc_curr, r2_std_curr,\
        mse_acc_prev, mse_std_prev, mse_acc_curr, mse_std_curr = decode_continuous_prev_cur(df_serial)

print('save file')
file1 = open("baselineAcc.txt", "w")
str_dictionary = repr(r2_acc_prev)
file1.write(str_dictionary + "\n")
str_dictionary = repr(r2_std_prev)
file1.write(str_dictionary + "\n")
str_dictionary = repr(r2_acc_curr)
file1.write(str_dictionary + "\n")
str_dictionary = repr(r2_std_curr)
file1.write(str_dictionary + "\n")
