...
################################# PARAMETER SETTINGS######################################################
    ...
    G_extern2inhib = 2.39*b2.nS#2.38 * b2.nS
    G_extern2excit = 3.2*b2.nS#3.1 * b2.nS
...
#################################### STIMULUS SETTINGS ###################################################
    ...
    t_distractor_end = numpy.zeros(distr_steps)*b2.ms
    for t in range(0, distr_steps):
    ...
    distr_center_idx = numpy.zeros(distr_steps)
    distr_target_idx = numpy.zeros(distr_steps)
    for k in range(0,distr_steps):
        distr_center_idx[k] = int(round(N_excitatory / 360. * distractor_center_deg[k]))
        distr_target_idx[k] = [idx % N_excitatory for idx in range(int(distr_center_idx[k]) - distr_width_idx, int(distr_center_idx[k]) + distr_width_idx + 1)][0]
    ...
    def stimulate_network(t):
        ...
        for k in range(0, distr_steps):
            if t >= t_distractor_start[k] and t < t_distractor_end[k]:
                excit_pop.I_stim[int(distr_target_idx[k])] = distractor_strength
...
def getting_started():
    ...
    distr_deg_max = 120
    num_steps = 15
    distr_deg = numpy.zeros(num_steps)
    distr_start = numpy.zeros(num_steps)
    for k in range(0,num_steps):
        distr_deg[k] = 180+(k+1)*distr_deg_max/num_steps
        distr_start[k] = 0+k*simtime/num_steps 
    a = distr_start*b2.second
    print(a)
    ...
    rate_monitor_excit, spike_monitor_excit, voltage_monitor_excit, idx_monitored_neurons_excit,\
        rate_monitor_inhib, spike_monitor_inhib, voltage_monitor_inhib, idx_monitored_neurons_inhib,\
        weight_profile\
        = simulate_wm(N_excitatory=N_e, N_inhibitory=256, poisson_firing_rate=2.4* b2.Hz, Jpos_excit2excit= 1.453,
                      weight_scaling_factor=2., sim_time=simtime, stimulus_center_deg=180,stimulus_width_deg=5, 
                      t_stimulus_start=0 * b2.ms, t_stimulus_duration=200 * b2.ms, stimulus_strength=0.8* b2.namp, 
                      distractor_center_deg=distr_deg, distr_steps = num_steps, distractor_width_deg=5, 
                      distractor_strength=0.8* b2.namp, t_distractor_start=distr_start*b2.second, 
