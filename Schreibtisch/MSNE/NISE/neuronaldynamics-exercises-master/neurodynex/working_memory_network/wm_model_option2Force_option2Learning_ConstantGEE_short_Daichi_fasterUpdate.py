...
matplotlib.rcParams.update({'font.size': 18})#14
...
def circdist(angle1, angle2, operation):
    if operation == '-':
        if angle1-angle2<0:
...
def get_results(files):
    ...
    for i,s1 in enumerate(s):
        try:
            results.append([float(s1.split(" ")[0]),float(s1.split(" ")[1]),float(s1.split(" ")[2]),float(s1.split(" ")[3]),float(s1.split(" ")[4]),float(s1.split(" ")[5])])
        except:
            continue
...
def simulate_wm(
        ...
        N_extern_poisson=1000, poisson_firing_rate=1.4 * b2.Hz, weight_scaling_factor=2.,grid = 0,
        sigma_weight_profile=20., Jpos_excit2excit=1.67, adapt = True, firstWO=False, error=180, prev_GEE_center=0*b2.nS, 
        prev_GEE_norm=0*b2.nS, prev_GEE_shift=0*b2.nS,norepi=0, NE_width = 40, num_washout = 0,
...
#################################### STIMULUS SETTINGS ###################################################
    ...
    NE_width_idx = int(round(N_excitatory / 360. * NE_width / 2))
...
######################################## EXCITATORY POPULATION ##########################################
    ...
    GEE = 0.35*0.381*b2.nS*weight_scaling_factor 
    
    # form subgroup for center neurons
    excit_center = excit_pop[int(stim_center_idx-NE_width_idx):int(stim_center_idx+NE_width_idx)]
    shift_center = (stimulus_center_deg-(distractor_center_deg-stimulus_center_deg))
    
    print(shift_center)
    print(NE_width)
    if ((shift_center-NE_width) < 0) | ((shift_center+NE_width) > 360) :
        print('Error, choose a smaller NE width')    
    else:
        excit_shiftcenter = excit_pop[int((shift_center-NE_width)*N_excitatory/360.):int((shift_center+NE_width)*N_excitatory/360.)]
...
######################################### NETWORK OPERATIONS #############################################
    ...
    @network_operation(dt=1 *b2.ms)
    def norepinephrin(t):
        K_increase = 0.001*b2.nS*(error-stimulus_center_deg)#0.0005*b2.nS*(error-stimulus_center_deg)
        K_decrease = 0.0005*b2.nS*(error-stimulus_center_deg)#*(error-(stimulus_center_deg-distractor_center_deg))
        K_decay = 0.005*b2.nS
        K_help = .001*b2.nS
        tau_incGEE_decNE = 1200*b2.ms
        tau_NE = 1000*b2.ms#2000*b2.ms
        tau_NE_decay = 4000*b2.ms#20000*b2.ms#12000*b2.ms
        fact=0.6
        # define rise/fall of Norepinephrin
        if adapt==True:
            excit_pop.G_excit2excit = (prev_GEE_norm-K_decrease*(1-numpy.exp(-t/tau_NE)))
            excit_center.G_excit2excit = (prev_GEE_center+K_increase*(1-numpy.exp(-t/tau_NE)))
        elif num_washout==0:
            excit_pop.G_excit2excit = (prev_GEE_norm-K_decrease*(1-numpy.exp(-t/tau_NE)))
            excit_shiftcenter.G_excit2excit = ((prev_GEE_shift+fact*(prev_GEE_center-GEE))+K_increase*(1-numpy.exp(-t/tau_NE))) #fact*prev_GEE_center-GEEfact*prev_GEE_center-GEE
        else:
            excit_pop.G_excit2excit = (GEE-K_help*(numpy.exp(-(t+sim_time*num_washout)/tau_incGEE_decNE)))
            excit_shiftcenter.G_excit2excit = (GEE+K_decay*(numpy.exp(-(t+sim_time*num_washout)/tau_NE_decay)))
            
        # grid[:,int(t/(100*b2.ms))] = excit_pop.G_excit2excit[:]
        # #print(numpy.amax(grid))
    ...
    return \
        excit_pop.G_excit2excit, excit_shiftcenter.G_excit2excit, excit_center.G_excit2excit, rate_monitor_excit, spike_monitor_excit, voltage_monitor_excit, idx_monitored_neurons_excit,\
        rate_monitor_inhib, spike_monitor_inhib, voltage_monitor_inhib, idx_monitored_neurons_inhib,\
        weight_profile_45
...
def getting_started():
    ...
    stimulus_center=180
    strength = 0.35*10**(-9) #0.35
    num_steps = 16
    distr_deg_max = 55
    grid = numpy.zeros((N_e, int(simtime/(100 *b2.ms))))
    ...
    result_file = "/home/melanie/Schreibtisch/MSNE/NISE/results/Norepinephrin0202_firstWOfixed3_partial06.txt"
    setmax = 5
    adaptation_len = 40
    washout_len = 31
    ...
    for sets in range(0,setmax):
        ...
        for trials in range(0,trialmax):
            ...
            if trials == 0:
                GEE_center = 0.35*0.381 * b2.nS*weight_factor
                GEE_norm = 0.35*0.381 * b2.nS*weight_factor
                GEE_shift = 0.35*0.381 * b2.nS*weight_factor
                err = stimulus_center
                adapt = True
                print('Adaptation trial')
            elif (trials < adaptation_len):
                file = get_results(result_file)
                GEE_center = file[(trials)-1+(sets)*trialmax, 3]*b2.nS
                GEE_norm = file[(trials)-1+(sets)*trialmax, 4]*b2.nS
                GEE_shift = file[(trials)-1+(sets)*trialmax, 5]*b2.nS
    ...
    ### Call working memory function ###                 
            G_ee_norm, G_ee_shift, G_ee_center, rate_monitor_excit, spike_monitor_excit, voltage_monitor_excit, idx_monitored_neurons_excit,\
                rate_monitor_inhib, spike_monitor_inhib, voltage_monitor_inhib, idx_monitored_neurons_inhib,\
                weight_profile\
                = simulate_wm(N_excitatory=N_e, N_inhibitory=256,
                              ...
                              error = err, prev_GEE_center = GEE_center, prev_GEE_norm = GEE_norm, prev_GEE_shift = GEE_shift, grid=grid,
                              learning_factor=learning_rate, adapt = adapt, NE_width =30, num_washout=num_washout,
                              ...
                              distractor_strength=d_strength, t_distractor_start=distr_start, 
...
#################################################### CALCULATE ERROR ##############################################################
            ...
            # if trials == 0:
            #     window = 10*b2.ms
            #     spikes=numpy.zeros((int(simtime/window),N_e))
            #     bump_c = numpy.zeros(int(simtime/window))
            #     for time in range(0, int(simtime/window)):
            #         for k in range(0,N_e):
            #             spikes[time,k] = numpy.where(i[numpy.where(numpy.logical_and(t>=time*window, t<time*window+10*b2.ms))[0]]==k)[0].size
            #         #print(numpy.where(spike_list[time,:]!=0))
            #         bump_c[time] = decode(spikes[time,:], N_e)
            #         #print(bump_center*360/(2*numpy.pi))
            ...
            #     plt.plot(t, i*360/N_e, 'k.', ms=3)
            #     plt.plot([0, simtime], [180,180], dashes=[6, 2], linewidth=7)
            #     plt.plot(numpy.linspace(0,simtime, int(simtime/window)), bump_c*180/numpy.pi , dashes=[1.5, 1], color='r', linewidth=6)
            #     plt.ylim([0,360])
            #     plt.xlabel('time [s]')
            ...
            #     plt.savefig('/home/melanie/Schreibtisch/MSNE/NISE/results/Drift0.png', bbox_inches='tight',  dpi=800)
            #     plt.show()
            # elif trials == 39:
            #     window = 50*b2.ms
            #     spikes=numpy.zeros((int(simtime/window),N_e))
            ...
            #     for time in range(0, int(simtime/window)):
            #         for k in range(0,N_e):
            #             spikes[time,k] = numpy.where(i[numpy.where(numpy.logical_and(t>=time*window, t<time*window+50*b2.ms))[0]]==k)[0].size
            #         #print(numpy.where(spike_list[time,:]!=0))
            #         bump_c[time] = decode(spikes[time,:], N_e)
            #         #print(bump_center*360/(2*numpy.pi))
            ...
            #     plt.figure(figsize=(10,10))
            #     plt.plot(t, i*360/N_e, 'k.', ms=3)
            #     plt.plot([0, simtime], [180,180], dashes=[6, 2], linewidth=10)
            #     plt.plot(numpy.linspace(0,simtime, int(simtime/window)), bump_c*180/numpy.pi , dashes=[3, 2], color='r', linewidth=10)
            #     plt.ylim([0,360])
            #     plt.xlabel('time [s]')
            #     plt.ylabel('neuron label [°]')
            #     #for k in range(0,num_steps):
            ...
            #     #for k in range(0,num_steps):
            #     #    plt.plot(distr_start[k], distr_deg[k]*N_e/(360), 'b.', MarkerSize=10)
            #     plt.savefig('/home/melanie/Schreibtisch/MSNE/NISE/results/Adaptation40.png', bbox_inches='tight',  dpi=800)
            #     plt.show()
            # elif trials == 40:
            #     window = 50*b2.ms
            #     spikes=numpy.zeros((int(simtime/window),N_e))
            ...
            #     #for k in range(0,num_steps):
            #     #    plt.plot(distr_start[k], distr_deg[k]*N_e/(360), 'b.', MarkerSize=10)
            #     plt.savefig('/home/melanie/Schreibtisch/MSNE/NISE/results/Washout1.png', bbox_inches='tight',  dpi=800)
            #     plt.show()
...
#################################################### SAVE TO FILE #################################################################
            ...
            with open(result_file, "a") as myfile:
                myfile.write(str(stimulus_center)+" "+str(distr_deg_max+180)+" "+str(bump_center*360/(2*numpy.pi))+" "+str(G_ee_center[int(len(G_ee_center)/2.)]/b2.nS)+" "+str(G_ee_norm[0]/b2.nS)+" "+str(G_ee_shift[int(len(G_ee_shift)/2.)]/b2.nS)+' '+str(learning_rate/b2.nS)+' '+str(setmax)+' '+str(trialmax)+"\n")
